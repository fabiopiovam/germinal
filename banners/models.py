...
class Photo(Sortable):
    ...
    def get_upload_to_image(self, filename):
        ...
        if ext == 'peg': ext='jpeg'        
        return 'banners/%s/%s_%s.%s' % (self.banner.id, datetime.now().strftime('%Y%m%d%H%M%S'), str(random.randint(00000,99999)), ext)
    
    def __unicode__(self):
        return u'%s' % self.image
    
    banner = SortableForeignKey('Banner')
...
class BannerActivatedManager(models.Manager):
    def get_queryset(self):
        return super(BannerActivatedManager, self).get_queryset().filter(published=True).order_by('-updated_at')


class Banner(Sortable):
    def __unicode__(self):
    ...
    def delete(self):
        dir = '%s/banners/%s' % (settings.MEDIA_ROOT, self.id)
    ...
    type = models.CharField(u'Banner', max_length=50, choices=banner_choices)
    ...
    activated   = BannerActivatedManager()
